...
### 📖 使用介紹
- 登入後預設顯示「使用介紹」頁：完整圖文操作說明與建議操作 6 步驟，方便新手上手
- 側邊欄依實際操作流程排列：使用介紹 → 教師 → 學生 → 課程 → 分組 → 簡易課表 → 總課表
...
- **Word (.docx)**：簡易課表、教師課表、學生課表、**總課表**、**教室統整課表**、**教室課表**
  - 總課表 Word 版面對照列印/PDF：**直向 A4**、星期反序、節次欄在右、分組由右到左、每格列出學生名單（該分組超過 10 位時自動兩人一行）
...
- 🧹 依程式碼審查修正登出與 restoreData 錯誤、清理殘留除錯 log 與無效設定
